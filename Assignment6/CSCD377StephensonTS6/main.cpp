...
void Display(void)
{
	...
	// Draws the table
	model_matrix = translate(mat4(1.0f), vec3(offset, 0.25, 0.0))*scale(mat4(1.0f), vec3(3.0, 0.25, 5.0));
	glUniformMatrix4fv(matrix_loc, 1, GL_FALSE, (GLfloat*)&model_matrix[0]);
	drawCube();

	// Draws the table legs
	model_matrix = translate(mat4(1.0f), vec3(offset - 2.5, -3.75, 2.5))*scale(mat4(1.0f), vec3(0.25, 3.5, 0.25));
	glUniformMatrix4fv(matrix_loc, 1, GL_FALSE, (GLfloat*)&model_matrix[0]);
	drawCube();

	//// Draws stool 1
	//model_matrix = translate(mat4(1.0f), vec3(offset - 2.5, -3.75, 2.5))*scale(mat4(1.0f), vec3(2.0, 2.0, 3.0));
	//glUniformMatrix4fv(matrix_loc, 1, GL_FALSE, (GLfloat*)&model_matrix[0]);
	//drawCube();

	// Draws the shade
	model_matrix = translate(mat4(1.0f), vec3(offset, 2.5, 0.0)) * scale(mat4(1.0f), vec3(5.0, 1.0, 5.0));
	glUniformMatrix4fv(matrix_loc, 1, GL_FALSE, (GLfloat*)&model_matrix[0]);
	drawShade();
